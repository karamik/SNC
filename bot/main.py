...
from bot.models.bank import DepositTariff, Deposit
from bot.middlewares.throttle import ThrottlingMiddleware
from bot.handlers import start, game, profile, admin, shop, tournament, referral, achievements, settings, clan
from bot.services.game_engine import GameEngine
from bot.services.scheduler import SchedulerService
from bot.services.events import EventManager
from bot.services.daily_bonus import process_daily_bonuses
from bot.services.tournament_manager import update_tournaments
from bot.services.bank import init_deposit_tariffs
...
async def init_achievements():
    ...
    async with async_session_maker() as session:
        ...
        session.add_all(achievements_list)
...
async def init_events():
    """Инициализация примеров ивентов."""
    async with async_session_maker() as session:
        count = await session.scalar(select(func.count(GameEvent.id)))
        if count > 0:
            return
        ...
        events = [
            GameEvent(
                name="Золотой час",
                description="Все выигрыши утроены!",
                event_type="multiplier",
                parameters='{"win_multiplier": 3.0, "duration_minutes": 10}',
            ...
            GameEvent(
                name="Чёрная дыра",
                description="Кэшбек повышен до 25%",
                event_type="cashback",
                parameters='{"cashback_percent": 25, "duration_minutes": 5}',
                start_time=now + datetime.timedelta(minutes=30),
                end_time=now + datetime.timedelta(minutes=35),
                is_active=True
            ),
        ]
        session.add_all(events)
        await session.commit()
        logger.info("Example events created")
...
async def setup_bot_commands():
    ...
    commands = [
        ...
        BotCommand(command="clan_join", description="Вступить в клан"),
        BotCommand(command="clan_leave", description="Покинуть клан"),
        BotCommand(command="clan_rankings", description="Топ кланов"),
        # Bank команды будут добавлены позже
    ]
...
async def main():
    global bot, dp, redis_client, engine, async_session_maker, game_engine, scheduler, event_manager
    ...
    await init_events()
    await init_deposit_tariffs(async_session_maker)  # Инициализация тарифов банка
    ...
    dp.include_router(clan.router)  # клановый роутер
    dp.include_router(admin.router)

    await setup_bot_commands()

    # Создаём менеджер событий и игровой движок
    event_manager = EventManager(async_session_maker)
    game_engine = GameEngine(bot, redis_client, async_session_maker, event_manager)
